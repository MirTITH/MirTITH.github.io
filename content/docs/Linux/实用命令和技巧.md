...
```bash
export dir=src/pinocchio_servo
export new_repo_url=<新仓库地址>

# 在主仓库中把这个目录拆成独立分支
git subtree split --prefix="$dir" -b temp_split

# 把这个分支推到新仓库
git remote add temp_origin "$new_repo_url"
git push temp_origin temp_split:main

# 从主仓库移除原目录
git rm -r "$dir"
git commit -m "Remove vendored $dir"

# 以子模块形式加回去
git submodule add "$new_repo_url" "$dir" # 注：这里也可以使用相对 URL
